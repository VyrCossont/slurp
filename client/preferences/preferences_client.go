...
func (a *Client) PreferencesGet(params *PreferencesGetParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*PreferencesGetOK, error) {
	...
	if params == nil {
		params = NewPreferencesGetParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "preferencesGet",
		Method:             "GET",
		PathPattern:        "/api/v1/preferences",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"http", "https"},
		Params:             params,
		Reader:             &PreferencesGetReader{formats: a.formats},
		AuthInfo:           authInfo,
	...
	success, ok := result.(*PreferencesGetOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for preferencesGet: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
