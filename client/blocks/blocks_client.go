...
// New creates a new blocks API client with basic auth credentials.
...
// New creates a new blocks API client with a bearer token for authentication.
// It takes the following parameters:
// - host: http host (github.com).
// - basePath: any base path for the API client ("/v1", "/v3").
// - scheme: http scheme ("http", "https").
...
func (a *Client) BlocksGet(params *BlocksGetParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*BlocksGetOK, error) {
	...
	if params == nil {
		params = NewBlocksGetParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "blocksGet",
		Method:             "GET",
		PathPattern:        "/api/v1/blocks",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"http", "https"},
		Params:             params,
		Reader:             &BlocksGetReader{formats: a.formats},
		AuthInfo:           authInfo,
	...
	success, ok := result.(*BlocksGetOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for blocksGet: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
