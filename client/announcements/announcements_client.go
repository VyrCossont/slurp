...
// New creates a new announcements API client with basic auth credentials.
...
// New creates a new announcements API client with a bearer token for authentication.
// It takes the following parameters:
// - host: http host (github.com).
// - basePath: any base path for the API client ("/v1", "/v3").
// - scheme: http scheme ("http", "https").
...
func (a *Client) AnnouncementsGet(params *AnnouncementsGetParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*AnnouncementsGetOK, error) {
	...
	if params == nil {
		params = NewAnnouncementsGetParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "announcementsGet",
		Method:             "GET",
		PathPattern:        "/api/v1/announcements",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"http", "https"},
		Params:             params,
		Reader:             &AnnouncementsGetReader{formats: a.formats},
		AuthInfo:           authInfo,
	...
	success, ok := result.(*AnnouncementsGetOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for announcementsGet: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
